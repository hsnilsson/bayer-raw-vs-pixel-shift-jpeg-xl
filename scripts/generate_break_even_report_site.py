...
def render_muimg_probe(
    ...
    <div class="note">
      ...
      <p><strong>Evidence scope:</strong> {esc(conclusion.get('not_proven', 'This probe covers one frame in camera-linear space.'))}</p>
    </div>
    <section class="questions">
      ...
      <div class="card"><h3>Low-distance calibration</h3><p>The d001 main codestream is byte-identical to d003 on this frame. Lossless is {float(next((item.get('mib', 0) for item in levels if isinstance(item, dict) and item.get('level') == 'lossless'), 0)):.2f} MiB; the first observed lossy plateau is about {float(next((item.get('mib', 0) for item in levels if isinstance(item, dict) and item.get('level') == 'd003'), 0)):.2f} MiB. Distance controls quality, while output size varies with image content; interpolation cannot target exactly 200 MiB.</p></div>
      <div class="card"><h3>Preserved in the checked file</h3><p>{preserved_text}.</p></div>
      <div class="card"><h3>Still missing</h3><p>{missing_text}. Lossy output requires recalculated raw-data IDs and digests.</p></div>
...
def crop_viewer_workspace(records: list[dict[str, object]]) -> str:
      ...
      <details class="crop-latitude" id="cropLatitude">
        ...
        <div class="crop-latitude-body">
          <div class="crop-tone-controls">
            <label class="tone-control"><span><strong>Exposure</strong><output>0.0 EV</output></span><input id="toneExposure" type="range" min="-4" max="4" step="0.1" value="0"></label>
            <label class="tone-control"><span><strong>Black</strong><output>0.000</output></span><input id="toneBlack" type="range" min="0" max="0.5" step="0.005" value="0"></label>
            <label class="tone-control"><span><strong>White</strong><output>1.000</output></span><input id="toneWhite" type="range" min="0.05" max="1" step="0.005" value="1"></label>
            <button type="button" id="toneReset">Reset edit</button>
          </div>
          <div class="crop-curve-wrap">
  ...
  (() => {
    ...
    const filmSidebar = workspace.querySelector(".crop-sidebar-left");
    const qualitySidebar = workspace.querySelector(".crop-sidebar-right");
    ...
    const toneCurve = document.getElementById("toneCurve");
    const toneCurveContext = toneCurve.getContext("2d");
    ...
    function setNavigationZone(zone, focusControl = false) {
      ...
      pinWorkspaceViewport();
    }

    function moveNavigationZone(delta) {
      const currentIndex = Math.max(0, navigationZones.indexOf(activeNavigationZone));
      const nextIndex = Math.max(0, Math.min(navigationZones.length - 1, currentIndex + delta));
      if (nextIndex !== currentIndex) setNavigationZone(navigationZones[nextIndex], true);
    }
    ...
    function curveValue(value) {
      ...
      const points = state.tone.points;
      for (let index = 1; index < points.length; index += 1) {
        if (x > points[index][0]) continue;
        const left = points[index - 1];
        const right = points[index];
        const amount = (x - left[0]) / Math.max(.00001, right[0] - left[0]);
    ...
    function drawToneCurve() {
      ...
      toneCurveContext.clearRect(0, 0, width, height);
      toneCurveContext.fillStyle = "#0b0e11";
      toneCurveContext.fillRect(0, 0, width, height);
      toneCurveContext.strokeStyle = "rgba(166,176,186,.18)";
      toneCurveContext.lineWidth = 1;
      for (let step = 0; step <= 4; step += 1) {
        const position = inset + plotWidth * step / 4;
        toneCurveContext.beginPath();
        toneCurveContext.moveTo(position, inset);
        toneCurveContext.lineTo(position, height - inset);
        toneCurveContext.stroke();
        const horizontal = inset + plotHeight * step / 4;
        toneCurveContext.beginPath();
        toneCurveContext.moveTo(inset, horizontal);
        toneCurveContext.lineTo(width - inset, horizontal);
        toneCurveContext.stroke();
      }
      toneCurveContext.setLineDash([5, 5]);
      toneCurveContext.strokeStyle = "rgba(166,176,186,.5)";
      toneCurveContext.beginPath();
      toneCurveContext.moveTo(inset, height - inset);
      toneCurveContext.lineTo(width - inset, inset);
      toneCurveContext.stroke();
      toneCurveContext.setLineDash([]);
      toneCurveContext.strokeStyle = "#7dd3fc";
      toneCurveContext.lineWidth = 2;
      toneCurveContext.beginPath();
      state.tone.points.forEach(([x, y], index) => {
        const px = inset + x * plotWidth;
        const py = height - inset - y * plotHeight;
        if (index) toneCurveContext.lineTo(px, py);
        else toneCurveContext.moveTo(px, py);
      });
      toneCurveContext.stroke();
      state.tone.points.forEach(([x, y], index) => {
        toneCurveContext.beginPath();
        toneCurveContext.arc(inset + x * plotWidth, height - inset - y * plotHeight, index === state.curvePoint ? 6 : 5, 0, Math.PI * 2);
        toneCurveContext.fillStyle = index === state.curvePoint ? "#fbbf24" : "#e7f6fd";
        toneCurveContext.fill();
        toneCurveContext.strokeStyle = "#075985";
        toneCurveContext.stroke();
      });
    }
    ...
    function nearestCurvePoint(event, radius = 14) {
      const rect = toneCurve.getBoundingClientRect();
      const [x, y] = curvePosition(event);
      let nearest = -1;
      let distance = Infinity;
      state.tone.points.forEach((point, index) => {
        const dx = (point[0] - x) * Math.max(1, rect.width - 24);
        const dy = (point[1] - y) * Math.max(1, rect.height - 24);
      ...
      return distance <= radius ? nearest : -1;
    }

    function moveCurvePoint(index, x, y) {
      if (index < 0 || index >= state.tone.points.length) return;
      if (index === 0) x = 0;
      else if (index === state.tone.points.length - 1) x = 1;
      else {
        const previousX = state.tone.points[index - 1][0];
        const nextX = state.tone.points[index + 1][0];
        x = Math.max(previousX + .005, Math.min(nextX - .005, x));
      }
      state.tone.points[index] = [x, clamp01(y)];
      drawToneCurve();
      scheduleToneRender();
    }
    ...
    function renderFilmList() {
      ...
      viewers.forEach((viewer, index) => {
        ...
        button.addEventListener("focus", () => { setNavigationZone("film"); });
    ...
    function renderQualityList() {
      ...
      viewer.candidates.forEach((candidate) => {
        ...
        button.addEventListener("click", () => {
          setNavigationZone("quality");
          setCandidate(candidate.key);
        });
        button.addEventListener("focus", () => { setNavigationZone("quality"); });
    ...
    function moveMode(delta) {
      ...
      if (nextIndex === currentIndex || !modes[nextIndex]) return;
      setMode(modes[nextIndex].key);
      setNavigationZone("view", true);
    }

    function clampLatitudePosition() {
      if (!latitude.classList.contains("is-floating")) return;
      const rect = latitude.getBoundingClientRect();
      ...
      latitude.style.left = `${left}px`;
      latitude.style.top = `${top}px`;
    }

    function setLatitudeFloating(enabled) {
      if (enabled) latitude.open = true;
      latitude.classList.toggle("is-floating", enabled);
      floatToggle.textContent = enabled ? "Dock" : "Pop out";
      floatToggle.setAttribute("aria-pressed", String(enabled));
      if (enabled) {
        latitude.style.left = `${Math.max(12, (window.innerWidth - latitude.offsetWidth) / 2)}px`;
        latitude.style.top = `${Math.max(12, (window.innerHeight - latitude.offsetHeight) / 2)}px`;
        clampLatitudePosition();
      } else {
    ...
    Object.entries(toneInputs).forEach(([key, input]) => {
      input.addEventListener("input", () => {
        ...
        input.closest("label").querySelector("output").value = `${Number(input.value).toFixed(key === "exposure" ? 1 : 3)}${suffix}`;
        scheduleToneRender();
      });
    });
    document.getElementById("toneReset").addEventListener("click", () => {
      const defaults = { exposure: 0, black: 0, white: 1 };
      Object.entries(defaults).forEach(([key, value]) => {
        state.tone[key] = value;
        toneInputs[key].value = value;
        toneInputs[key].dispatchEvent(new Event("input"));
      });
      state.tone.points = [[0, 0], [.25, .25], [.5, .5], [.75, .75], [1, 1]];
      state.curvePoint = -1;
      drawToneCurve();
      scheduleToneRender();
    });
    toneCurve.addEventListener("pointerdown", (event) => {
      if (event.button !== 0) return;
      event.preventDefault();
      const [x, y] = curvePosition(event);
      let index = nearestCurvePoint(event);
      if (index < 0) {
        state.tone.points.push([x, y]);
        state.tone.points.sort((left, right) => left[0] - right[0]);
        index = state.tone.points.findIndex((point) => point[0] === x && point[1] === y);
      }
      state.curvePoint = index;
      moveCurvePoint(index, x, y);
      toneCurve.setPointerCapture(event.pointerId);
    });
    toneCurve.addEventListener("pointermove", (event) => {
      if (state.curvePoint < 0 || !toneCurve.hasPointerCapture(event.pointerId)) return;
      const [x, y] = curvePosition(event);
      moveCurvePoint(state.curvePoint, x, y);
    });
    const finishCurveDrag = (event) => {
      if (toneCurve.hasPointerCapture(event.pointerId)) toneCurve.releasePointerCapture(event.pointerId);
      state.curvePoint = -1;
      drawToneCurve();
    };
    toneCurve.addEventListener("pointerup", finishCurveDrag);
    toneCurve.addEventListener("pointercancel", finishCurveDrag);
    toneCurve.addEventListener("contextmenu", (event) => {
      event.preventDefault();
      const index = nearestCurvePoint(event);
      if (index <= 0 || index >= state.tone.points.length - 1) return;
      state.tone.points.splice(index, 1);
      state.curvePoint = -1;
      drawToneCurve();
      scheduleToneRender();
    });
    floatToggle.addEventListener("click", (event) => {
      ...
      setLatitudeFloating(!latitude.classList.contains("is-floating"));
    });
    latitudeHandle.addEventListener("pointerdown", (event) => {
      if (!latitude.classList.contains("is-floating") || event.button !== 0 || event.target.closest("button")) return;
      const rect = latitude.getBoundingClientRect();
      state.movingLatitude = true;
      state.latitudeMoved = false;
      state.latitudeOffsetX = event.clientX - rect.left;
      state.latitudeOffsetY = event.clientY - rect.top;
      latitudeHandle.setPointerCapture(event.pointerId);
    });
    latitudeHandle.addEventListener("pointermove", (event) => {
      if (!state.movingLatitude || !latitudeHandle.hasPointerCapture(event.pointerId)) return;
      state.latitudeMoved = true;
      const left = Math.max(8, Math.min(window.innerWidth - latitude.offsetWidth - 8, event.clientX - state.latitudeOffsetX));
      const top = Math.max(8, Math.min(window.innerHeight - latitude.offsetHeight - 8, event.clientY - state.latitudeOffsetY));
      latitude.style.left = `${left}px`;
      latitude.style.top = `${top}px`;
    });
    const finishLatitudeMove = (event) => {
      state.movingLatitude = false;
      if (latitudeHandle.hasPointerCapture(event.pointerId)) latitudeHandle.releasePointerCapture(event.pointerId);
    };
    latitudeHandle.addEventListener("pointerup", finishLatitudeMove);
    latitudeHandle.addEventListener("pointercancel", finishLatitudeMove);
    latitudeHandle.addEventListener("click", (event) => {
      if (!state.latitudeMoved) return;
      event.preventDefault();
      state.latitudeMoved = false;
    });
    latitude.addEventListener("toggle", () => {
      if (!latitude.open && latitude.classList.contains("is-floating")) setLatitudeFloating(false);
      window.requestAnimationFrame(() => {
        resizeCanvas();
        draw();
        drawToneCurve();
...
def render_html(
    ...
    if identity_color_values and stress_color_values:
        ...
        stress_result = (
            ...
            "project's DeltaE00 3 review threshold. The hard inversion therefore amplifies RAW61-versus-PS16 "
            "workflow differences. JPEG XL codec loss is measured separately against the PS16 reference."
        )
    else:
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .review-keyboard-hint {{ display: block; margin-top: 8px; padding: 8px 10px; border: 2px solid #b45309; border-radius: 8px; background: #fef3c7; color: #422006; font-weight: 800; text-decoration: underline; text-decoration-thickness: 2px; text-underline-offset: 3px; }}
    .review-keyboard-hint kbd {{ display: inline-block; min-width: 1.8em; margin: 0 1px; padding: 1px 5px; border: 1px solid #92400e; border-radius: 4px; background: #fff; color: #111827; box-shadow: 0 2px 0 #92400e; font: inherit; text-align: center; text-decoration: none; }}
    ...
    .crop-sidebar[data-navigation-active="true"] {{ box-shadow: inset 0 0 0 2px #7dd3fc; }}
    .crop-sidebar-header {{
      position: sticky;
      top: 0;
      z-index: 1;
    ...
    @media (max-width: 700px) {{
      ...
      .crop-workspace {{ grid-template-rows: auto minmax(62vh, 540px) auto; border-radius: 8px; }}
      .crop-sidebar {{ max-height: 142px; padding: 10px; }}
      ...
      .crop-latitude.is-floating .crop-latitude-body {{ grid-template-columns: 1fr; }}
      .crop-latitude.is-floating .crop-histogram-wrap {{ grid-column: 1; }}
  ...
  <main>
    ...
    <section class="questions">
      <div class="card question-card">
        ...
        <details>
          ...
          <p>The tested lossy levels show very small patch color movement against the PS16 reference. A trained eye can still see changes in grain and texture, so the useful result is the measured quality boundary across compression levels.</p>
        </details>
      </div>
      <div class="card question-card">
      ...
      <div class="card question-card">
        ...
        <details>
          <summary>Answer so far</summary>
          <p>{current_conclusion} The levels currently below the RAW61 median-size budget are {esc(zone_text)}. Color, structure, and visual evidence remain separate decision gates, and broader film coverage is required for a general recommendation.</p>
        </details>
      </div>
      <div class="card question-card">
    ...
    <details class="adc-disclosure">
      ...
      <div class="adc-disclosure-body">
        <p><strong>How to read the list:</strong> “confirmed” records something observed in the local files or application probe. “Open validation” identifies evidence that would be required before reconsidering this path and sits outside the current work queue.</p>
      ...
      <div class="adc-item">
        ...
        <dl>
          ...
          <dt>Required before reconsideration</dt>
          <dd>Use each file's declared scale, apply its required opcodes, then test clipping and recoverable latitude through the same trusted renderer. The current low-level normalization supports diagnostic comparison; restoration of the original raw-scale semantics requires the complete render path.</dd>
        </dl>
      </div>
      <div class="adc-item">
      ...
      <div class="adc-item">
        ...
        <dl>
          ...
          <dt>Actual problem</dt>
          <dd>The lossy path preserves the image through a perceptual transform rather than camera-native linear channel samples. Its response may differ under negative inversion, channel balancing, or future extreme edits.</dd>
          <dt>Required before reconsideration</dt>
      ...
      <div class="adc-item">
        ...
        <dl>
          ...
          <dt>Actual problem</dt>
          <dd>The fixed RawTherapee pipeline currently stops at JPEG XL-compressed image data inside DNG 1.7. The embedded JXL pixels decode independently, and the application finding is limited to this raw-loading path.</dd>
          <dt>Required before reconsideration</dt>
      ...
      <div class="adc-item">
        ...
        <dl>
          ...
          <dd>This custom low-level check covers tile extraction, pixel decoding, and selected metadata. Consistent interpretation of crops, color matrices, opcodes, previews, and metadata still requires an independent full DNG implementation.</dd>
          <dt>Required before reconsideration</dt>
      ...
      <div class="adc-item">
        ...
        <dl>
          ...
          <dt>Actual missing evidence</dt>
          <dd>The missing evidence is a blinded, end-to-end comparison after a real color-managed inversion and grading workflow. That review would test whether low average patch error still permits objectionable grain, texture, or local-density changes.</dd>
          <dt>Required before reconsideration</dt>
      ...
      <div class="adc-item">
        ...
        <dl>
          ...
          <dt>Actual problem</dt>
          <dd>The tested conservative ADC levels reduce PS16 storage substantially but remain larger than the RAW61 budget, leaving the same-storage question open for this path.</dd>
          <dt>Required before reconsideration</dt>
