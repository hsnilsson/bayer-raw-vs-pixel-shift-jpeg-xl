...
class CompareDngDuplicateCandidatesTests(unittest.TestCase):
    def test_canonical_stem_for_duplicate(self) -> None:
        self.assertEqual(
            duplicate_compare.canonical_stem_for_duplicate("_DSC1000-_DSC1015-(1)"),
            "_DSC1000-_DSC1015",
        )
        self.assertIsNone(duplicate_compare.canonical_stem_for_duplicate("_DSC1000-_DSC1015"))
    ...
    def test_discover_duplicate_pairs_matches_root_canonical(self) -> None:
        ...
        write_file(root / "_review" / "duplicate_dng_candidates" / "_DSC2000-_DSC2015-(1).dng")

        pairs = duplicate_compare.discover_duplicate_pairs(root)

        self.assertEqual(len(pairs), 1)
        self.assertEqual(pairs[0].canonical.name, "_DSC1000-_DSC1015.dng")
        self.assertEqual(pairs[0].duplicate.name, "_DSC1000-_DSC1015-(1).dng")
