...
class BreakEvenPipelineTests(unittest.TestCase):
    ...
    def test_report_site_includes_bounded_muimg_probe(self) -> None:
        ...
        self.assertIn("could not decode JPEG XL image data inside DNG", html)
        self.assertIn("same-render end-to-end comparison remains unavailable", html)
    ...
    def test_report_site_embeds_inline_crop_viewer_manifest(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn('copy.className = "crop-film-copy";', html)
            ...
            self.assertIn('modeSelect.addEventListener("focus", () => { setNavigationZone("view"); });', html)
            self.assertIn('button.addEventListener("focus", () => { setNavigationZone("quality"); });', html)
            self.assertIn('role.textContent = [roleLabel, sizeLabel].filter(Boolean).join(" | ");', html)
            self.assertIn('if (event.key === "ArrowRight")', html)
            self.assertIn("moveNavigationZone(1)", html)
            self.assertIn('if (event.key === "ArrowDown")', html)
            self.assertIn('if (activeNavigationZone === "view") moveMode(1);', html)
            self.assertIn(
                'function setCandidate(key) {\n      state.candidateKey = key;\n      renderQualityList();',
                html,
            )
            self.assertIn('meta.textContent = `${parts.join(" | ")}${mode.description', html)
            self.assertIn('-webkit-line-clamp: 4;', html)
            self.assertIn('class="crop-toolbar-copy"', html)
            self.assertIn('height: calc(1.45em * 7);', html)
            self.assertIn('scrollbar-gutter: stable;', html)
            self.assertIn('.crop-actions { width: 240px;', html)
            self.assertIn('.crop-toolbar p { height: calc(1.45em * 5); }', html)
            self.assertIn('background: #0f1215;', html)
            self.assertIn('overflow: clip;', html)
            self.assertIn('.crop-sidebar[data-navigation-active="true"]', html)
            ...
            self.assertIn('function resizeToneCurveCanvas() {', html)
            self.assertIn('toneCurveContext.setTransform(ratio, 0, 0, ratio, 0, 0);', html)
            self.assertIn('Math.round(toneCurve.clientWidth * ratio)', html)
            self.assertIn('Math.round(toneCurve.clientHeight * ratio)', html)
            self.assertIn('const width = toneCurve.clientWidth;', html)
            self.assertIn('const insetX = 12;', html)
            self.assertIn('latitude.style.removeProperty("width");', html)
            self.assertIn('latitude.style.removeProperty("height");', html)
            self.assertIn('.crop-stage { min-width: 0; min-height: 0;', html)
            self.assertIn('<div class="crop-canvas-wrap">', html)
            self.assertIn('.crop-canvas-wrap { position: relative;', html)
            self.assertIn('.crop-status { position: absolute;', html)
            self.assertIn('.crop-status:empty { opacity: 0; }', html)
            self.assertIn('grid-template-rows: auto minmax(0, 1fr) auto;', html)
            self.assertIn('grid-template-columns: minmax(55px, 240px) minmax(85px, 420px);', html)
            self.assertIn('min-width: 55px; max-width: 240px;', html)
            self.assertIn('min-width: 85px; max-width: 420px;', html)
            self.assertIn('width: min(720px, 100%);', html)
            self.assertIn('min-width: 75px;', html)
            ...
            self.assertIn('class="review-keyboard-hint"', html)
            self.assertIn('<kbd>←</kbd>/<kbd>→</kbd>', html)
            self.assertIn('<kbd>↑</kbd>/<kbd>↓</kbd>', html)
            self.assertIn('press <kbd>O</kbd> to toggle the overlay.', html)
            self.assertIn('text-decoration: underline;', html)
            self.assertIn('background: #fef3c7;', html)
