...
storage-budget reference. The verified report measures and displays four
f/4.5 crops at ten standalone JXL levels and five fixed transformations in
the `secondary_same_sequence_target` cohort. An independent RAW61 pair is
still needed before this capture can enter the primary storage-budget comparison.
The crop locations in
[`metadata/adox_vlad_f45_crop_plan.json`](../metadata/adox_vlad_f45_crop_plan.json)
and [`metadata/verified_experiment.json`](../metadata/verified_experiment.json)
were updated together from the owner's annotated context map on 2026-09-17.
The DNG-derived PS16 render and standalone JXL levels are compared with the
aligned first ARW from the same sequence. The older f/8 numeric and combiner
results remain historical and use their separate crop selections.
